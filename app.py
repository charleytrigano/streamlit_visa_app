...
# --- Persistance chemin fichier
def _load_last_path() -> Path|None:
    try:
        if STATE_FILE.exists():
            p = Path(json.loads(STATE_FILE.read_text(encoding="utf-8")).get("last_path",""))
...
def _save_last_path(p: Path) -> None:
    try:
        STATE_FILE.write_text(json.dumps({"last_path": str(p)}, ensure_ascii=False), encoding="utf-8")
    except:
        pass
...
def current_file_path() -> Path|None:
    ...
    return Path(p) if p and Path(p).exists() else _load_last_path()

def set_current_file_from_upload(up) -> Path|None:
    if up is None: 
        return None
    name = up.name or "donnees_visa_clients.xlsx"
    buf = up.getvalue() if hasattr(up,"getvalue") else up.read()
    ...
    try:
        ...
        st.error(f"Impossible d’enregistrer le fichier: {e}")
        return None
...
def _to_num(s:Any)->pd.Series:
    ...
    if isinstance(s,pd.DataFrame):
        s = s.iloc[:,0] if s.shape[1] else pd.Series(dtype=float, index=s.index)
...
def read_sheet(path:Path, sheet:str, normalize=False)->pd.DataFrame:
    try: 
...
def _col_or_empty(df: pd.DataFrame, col_name: str) -> pd.Series:
    """Retourne df[col_name] nettoyé, ou une Series '' de la bonne longueur si la colonne n'existe pas."""
...
def normalize_dataframe(df:pd.DataFrame)->pd.DataFrame:
    ...
    base_cols=[DOSSIER_COL,"ID_Client","Nom","Catégorie","Visa",HONO,AUTRE,TOTAL,"Payé","Reste",PAY_JSON,"Date","Mois",ESC_TR,ESC_JR]+STATUS_COLS+STATUS_DATES
    for c in base_cols:
    ...
    paid_json=[_sum_payments(_parse_json_list(r.get(PAY_JSON,""))) for _,r in df.iterrows()]
    paid_json=pd.Series(paid_json, index=df.index, dtype=float)
    df["Payé"]=pd.Series([max(a,b) for a,b in zip(_to_num(df["Payé"]), paid_json)], index=df.index)
...
def read_visa_reference_tree(path: Path) -> pd.DataFrame:
    ...
    try:
        ...
        if not isinstance(base, pd.DataFrame):
            return pd.DataFrame(columns=REF_COLS + ["Visa_base"])
    except Exception:
        return pd.DataFrame(columns=REF_COLS + ["Visa_base"])
    ...
    df["Catégorie"] = _col_or_empty(base, col_cat)
    df["SC1"]       = _col_or_empty(base, col_s1)
    df["SC2"]       = _col_or_empty(base, col_s2)
    df["SC3"]       = _col_or_empty(base, col_s3)
    df["SC4"]       = _col_or_empty(base, col_s4)
    df["Visa"]      = _col_or_empty(base, col_v)
...
def visas_autorises_from_tree(df_ref: pd.DataFrame, sel: dict) -> list[str]:
    ...
    def _apply(key,col):
        ...
        if val:
            idx=sub[col].astype(str)==val
            if idx.any(): sub=sub[idx]
    for k,c in [("Catégorie","Catégorie"),("SC1","SC1"),("SC2","SC2"),("SC3","SC3"),("SC4","SC4")]:
        _apply(k,c)
    if _safe_str(sel.get("Visa","")):
        sub=sub[sub["Visa"]==sel["Visa"]]
    return sorted([v for v in sub["Visa_base"].unique() if v])
...
def filter_by_selection(df: pd.DataFrame, sel: dict, df_ref_tree: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    col_vb  = "Visa_base" if "Visa_base" in f.columns else _find_col(f, ["Visa"])
    f["__cat"] = f[col_cat].astype(str).map(_norm_txt) if col_cat else ""
    f["__vb"]  = f[col_vb].astype(str).map(_norm_txt) if col_vb else ""
    want_cat = _norm_txt(sel.get("Catégorie",""))
    want_vb  = _norm_txt(_visa_base(sel.get("Visa","")))
    if want_cat:
        f = f[f["__cat"]==want_cat]
    if want_vb:
        f = f[f["__vb"]==want_vb]
    else:
        if df_ref_tree is not None:
            wl = {_norm_txt(v) for v in visas_autorises_from_tree(df_ref_tree, sel)}
            if wl:
                f = f[f["__vb"].isin(wl)]
    return f.drop(columns=[c for c in f.columns if c.startswith("__")], errors="ignore")


# ============= VISA APP — PARTIE 2/5 (COMPACTE) =============
st.set_page_config(page_title="Visa Manager", layout="wide")

# --- Sidebar : chargement du fichier Excel
...
path_text = st.sidebar.text_input("Ou saisir un chemin local", value=st.session_state.get("current_path",""))
col_a, col_b = st.sidebar.columns(2)
if col_a.button("📄 Ouvrir", key="btn_open"):
    p = Path(path_text)
    if p.exists():
        save_workspace_path(p)
        st.sidebar.success(f"Ouv. : {p.name}")
        st.rerun()
    else:
        st.sidebar.error("Chemin invalide.")
if col_b.button("♻️ Reprendre dernier", key="btn_last"):
    p = _load_last_path()
    if p:
        save_workspace_path(p)
...
if current_path is None:
    st.warning("Aucun fichier .xlsx sélectionné.")
    st.stop()

# --- Feuilles du fichier
sheets = list_sheets(current_path)
if not sheets:
    st.error("Classeur illisible ou vide.")
    st.stop()
st.sidebar.write("**Feuilles détectées :**")
for i, sn in enumerate(sheets):
    st.sidebar.write(f"- {i+1}. {sn}")

# Feuille principale = celle contenant 'Nom' et 'Visa'
client_target_sheet = None
for sn in sheets:
    df_test = read_sheet(current_path, sn, normalize=False)
    if {"Nom","Visa"}.issubset(set(df_test.columns.astype(str))):
...
# --- Référentiel Visa (Catégorie, SC1..SC4, Visa)
...
with tab_dash:
    ...
    st.markdown("### 🔎 Filtres hiérarchiques")
    cL, cR = st.columns([1, 2])
    show_all = cL.checkbox("Afficher tous les dossiers", value=False, key=f"dash_showall_{sheet_choice}")

    # Sélecteur hiérarchique Catégorie → SC1..SC4 → Visa
    def cascading_visa_picker_tree(df_ref: pd.DataFrame, key_prefix: str) -> dict:
        sel = {"Catégorie":"","SC1":"","SC2":"","SC3":"","SC4":"","Visa":""}
        if df_ref.empty:
            cL.info("Référentiel vide."); return sel
        # Catégorie
        cats = sorted([v for v in df_ref["Catégorie"].unique() if v])
        sel["Catégorie"] = cL.selectbox("Catégorie", [""]+cats, key=f"{key_prefix}_cat")
        sub = df_ref[df_ref["Catégorie"]==sel["Catégorie"]] if sel["Catégorie"] else df_ref.copy()
        # SC1→SC4
        for lvl in ["SC1","SC2","SC3","SC4"]:
            opts = sorted([v for v in sub[lvl].unique() if v])
            sel[lvl] = cL.selectbox(lvl, [""]+opts, key=f"{key_prefix}_{lvl}")
            if sel[lvl]:
                sub = sub[sub[lvl]==sel[lvl]]
        visas = sorted([v for v in sub["Visa"].unique() if v])
        sel["Visa"] = cL.selectbox("Visa", [""]+visas, key=f"{key_prefix}_visa")
        return sel

    sel_path = cascading_visa_picker_tree(visa_ref_tree, f"tree_{sheet_choice}")
    ...
    sel_years = c1.multiselect("Année", years, default=[], key=f"year_{sheet_choice}")
    sel_months = c2.multiselect("Mois (MM)", months, default=[], key=f"month_{sheet_choice}")
    include_na = c3.checkbox("Inclure sans date", value=True, key=f"na_{sheet_choice}")

    # --- Application des filtres
    f = df.copy()
    if not show_all:
        f = filter_by_selection(f, sel_path, df_ref_tree=visa_ref_tree)
    if "Date" in f.columns and sel_years:
        mask = f["Date"].apply(lambda x: pd.notna(x) and x.year in sel_years)
        if include_na: mask |= f["Date"].isna()
        f = f[mask]
    if "Mois" in f.columns and sel_months:
        mask = f["Mois"].isin(sel_months)
        if include_na: mask |= f["Mois"].isna()
        f = f[mask]

    st.caption(f"{len(f)} dossiers affichés / {len(df)} totaux")

    # --- KPI
    st.markdown("""
    <style>
      .small-kpi [data-testid="stMetricValue"]{font-size:1.2rem}
      .small-kpi [data-testid="stMetricLabel"]{font-size:.8rem;opacity:.7}
    </style>
    ...
    cols = [c for c in [DOSSIER_COL,"ID_Client","Nom","Date","Mois","Catégorie","Visa",
                        HONO,AUTRE,TOTAL,"Payé","Reste",
                        S_ENVOYE,D_ENVOYE,S_APPROUVE,D_APPROUVE,
                        S_RFE,D_RFE,S_REFUSE,D_REFUSE,S_ANNULE,D_ANNULE]
             if c in f.columns]
    view = f.copy()
    for c in [HONO,AUTRE,TOTAL,"Payé","Reste"]:
        if c in view.columns:
            view[c] = pd.to_numeric(view[c], errors="coerce").fillna(0.0).map(_fmt_money_us)
...
with tab_analyses:
    ...
    with st.container():
        ...
        def cascade_anal(df_ref: pd.DataFrame, key_prefix: str) -> dict:
            sel = {"Catégorie":"","SC1":"","SC2":"","SC3":"","SC4":"","Visa":""}
            if df_ref is None or df_ref.empty:
                cL.info("Référentiel Visa vide."); return sel
            cats = sorted([v for v in df_ref["Catégorie"].unique() if v])
            sel["Catégorie"] = cL.selectbox("Catégorie", [""]+cats, key=f"{key_prefix}_cat")
            sub = df_ref[df_ref["Catégorie"]==sel["Catégorie"]] if sel["Catégorie"] else df_ref.copy()
            for lvl,label in [("SC1","Sous-catégorie 1"),("SC2","Sous-catégorie 2"),("SC3","Sous-catégorie 3"),("SC4","Sous-catégorie 4")]:
                opts = sorted([v for v in sub[lvl].unique() if v])
                sel[lvl] = cL.selectbox(label, [""]+opts, key=f"{key_prefix}_{lvl}")
                if sel[lvl]: sub = sub[sub[lvl]==sel[lvl]]
            visas = sorted([v for v in sub["Visa"].unique() if v])
            sel["Visa"] = cL.selectbox("Visa", [""]+visas, key=f"{key_prefix}_visa")
            return sel
    ...
    # ---------------- KPI ----------------
    st.markdown("""
    <style>.small-kpi [data-testid="stMetricValue"]{font-size:1.15rem}
    ...
    if alt is not None and not by_ym.empty:
        try:
            ...
            melt2 = by_ym.melt(["Année","MoisNum"], ["Total","Payé","Reste"],
                               var_name="Indicateur", value_name="Montant")
            c2.altair_chart(
    ...
    top_vis = (by_v.groupby("Visa")["Total"].sum()
    ...
    for c in [HONO, AUTRE, TOTAL, "Payé", "Reste"]:
        if c in det.columns:
            det[c] = det[c].apply(lambda x: _fmt_money_us(x) if pd.notna(x) else "")
...
with tab_escrow:
    ...
    if len(to_claim):
        ...
        cols_alert = [DOSSIER_COL,"ID_Client","Nom","Visa",HONO,"Payé","Dispo ESCROW"]
        st.dataframe(to_claim[[c for c in cols_alert if c in to_claim.columns]], use_container_width=True)

    st.divider()
    st.markdown("### 🔁 Marquer un transfert ESCROW → Compte ordinaire")

    # Un expander par dossier avec dispo > 0
    for i, r in dfE.iterrows():
        ...
        with st.expander(header, expanded=False):
            ...
            if st.button("💾 Enregistrer le transfert", key=f"esc_save_{sheet_choice}_{i}"):
                raw = read_sheet(current_path, client_target_sheet, normalize=False)
                # Sécurité: si l'index a changé après filtrages/tri éventuels
                if i >= len(raw):
                    st.error("Ligne introuvable (index décalé). Rouvrez la page et réessayez.")
                    st.stop()
                ...
                try:
                    row[ESC_TR] = float(row.get(ESC_TR, 0.0) or 0.0) + float(amt)
                except Exception:
                    row[ESC_TR] = float(amt)
