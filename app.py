...
with tabs[0]:
    ...
    if "Date" in view.columns:
        # afficher YYYY-MM-DD, sans l'heure
        try:
    ...
    # Tri sûr : on ne garde que les clés existantes
...
def _render_status_dates_block(row: pd.Series):
    """Bloc statuts + dates — version sans f-strings pour éviter les quotes piégeuses."""
...
with tabs[4]:
    ...
    if df_all.empty:
        ...
        # Sélection par Nom OU par ID_Client
        ...
        if not isinstance(hist, list):
            try:
                ...
                hist = []
        # Affichage historique (lecture seule)
        if hist:
        ...
        if go_add:
            if pay_amt <= 0:
                ...
                # Recharge fichier, sécurise l’index, met à jour, réécrit
                ...
                if not mask.any():
                    ...
                    # Recalcul Payé/Reste
                    # Payé = max(ancienne colonne Payé, somme JSON Paiements)
                    def _sum_json(lst):
                        try:
                            return float(sum(float(it.get("amount",0.0) or 0.0) for it in (lst or [])))
                        except Exception:
...
with tabs[3]:
    ...
    # Aperçu brut du mapping
    with st.expander("Voir le mapping brut (cat → sous-cat → options)"):
        st.json(visa_map)

    # Petit sélecteur pour tester le rendu des options
    if visa_map:
        ...
        if test_cat:
            ...
            if test_sub:
                ...
                # On affiche les options disponibles dans cette sous-catégorie
...
with tabs[2]:
    ...
    if df_all.empty:
        ...
        st.caption("NB : pour un escrow « stricte », on peut isoler les honoraires perçus avant envoi, puis signaler les transferts à effectuer une fois le statut « Dossier envoyé » coché.")


# ==============================================
# 🧾 ONGLET : Gestion (Ajouter / Modifier / Supprimer)
# ==============================================
def _visa_cascade_ui(visa_map: dict, ns_prefix: str, preset_cat: str = "", preset_sub: str = "", preset_opts: dict | None = None):
    """
    Cascade Catégorie → Sous-catégorie → options (radio/checkbox)
    Retourne (cat, sub, visa_final, opts_dict, info_msg)
    """
    cats = sorted(list(visa_map.keys()))
    cat = st.selectbox("Catégorie", [""] + cats, index=(cats.index(preset_cat)+1 if preset_cat in cats else 0),
                       key=f"{ns_prefix}_cat")
    sub = ""
    visa_final = ""
    ...
    if cat:
        ...
        sub = st.selectbox("Sous-catégorie", [""] + subs,
                           ...
                           key=f"{ns_prefix}_sub")
        if sub:
            visa_final, opts_dict, info_msg = build_visa_option_selector(
                visa_map, cat, sub, keyprefix=f"{ns_prefix}_opts", preselected=preset_opts or {}
            )
    return cat, sub, visa_final, opts_dict, info_msg

with st.sidebar:
    st.subheader("🧾 Gestion rapide (dans l’onglet)")
    st.caption("Utilisez l’onglet **Gestion** ci-dessous pour appliquer les changements.")
...
with tabs[4]:
    ...
    if op == "Ajouter":
        ...
        cat, sub, visa_final, opts_dict, info_msg = _visa_cascade_ui(visa_map, ns_prefix=f"add_{SID}")
        if info_msg:
            st.info(info_msg)
        ...
        if df_live.empty:
            ...
            dt  = c2.date_input("Date de création", value=(row.get("Date") if isinstance(row.get("Date"), date) else date.today()), key=f"mod_date_{SID}")
            mois = c3.selectbox("Mois (MM)", [f"{m:02d}" for m in range(1,13)],
            ...
            st.markdown("#### 🎯 Choix Visa")
            cat, sub, visa_final, opts_dict, info_msg = _visa_cascade_ui(
                visa_map, ns_prefix=f"mod_{SID}",
            ...
            sent_d = s1.date_input("Date d'envoi", value=(row.get("Date d'envoi") if isinstance(row.get("Date d'envoi"), date) else None), key=f"mod_sentd_{SID}")
            acc    = s2.checkbox("Dossier accepté", value=bool(row.get("Dossier accepté")), key=f"mod_acc_{SID}")
            acc_d  = s2.date_input("Date d'acceptation", value=(row.get("Date d'acceptation") if isinstance(row.get("Date d'acceptation"), date) else None), key=f"mod_accd_{SID}")
            ref    = s3.checkbox("Dossier refusé", value=bool(row.get("Dossier refusé")), key=f"mod_ref_{SID}")
            ref_d  = s3.date_input("Date de refus", value=(row.get("Date de refus") if isinstance(row.get("Date de refus"), date) else None), key=f"mod_refd_{SID}")
            ann    = s4.checkbox("Dossier annulé", value=bool(row.get("Dossier annulé")), key=f"mod_ann_{SID}")
            ann_d  = s4.date_input("Date d'annulation", value=(row.get("Date d'annulation") if isinstance(row.get("Date d'annulation"), date) else None), key=f"mod_annd_{SID}")
    ...
    if st.button("Préparer l’archive ZIP", key=f"zip_btn_{SID}"):
        try:
            ...
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                # Visa (fichier tel quel)
                try:
                    zf.write(visa_path, "Visa.xlsx")
                except Exception:
                    # Si inaccessible, tenter une lecture puis réécriture
                    try:
